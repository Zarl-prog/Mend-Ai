  ...
  return (
    <>
      <div className="mobile-properties fixed bottom-14 left-2 right-2 z-20">
        <div className="h-11 bg-card border border-card rounded-t-xl flex items-center justify-around px-2">
          <button
            onClick={() => setShowColorPicker(true)}
            className="flex-1 flex flex-col items-center justify-center h-full text-secondary hover:text-body"
          >
            <div className="w-5 h-5 rounded border border-card" style={{ backgroundColor: selectedShape.fillColor }} />
          ...
          <button
            ...
            className="flex-1 flex flex-col items-center justify-center h-full text-[#FC8181] hover:text-body"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      >
          ...
          {presetColors.map(color => (
            <button
              ...
              className={`w-full aspect-square rounded-lg border-2 ${
                selectedShape.fillColor === color ? 'border-white' : 'border-card'
              }`}
        ...
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            value={selectedShape.fillColor}
            onChange={(e) => handleColorSelect(e.target.value)}
            className="flex-1 bg-hover text-body text-sm px-3 py-2 rounded border border-card"
          />
        </div>
      </BottomSheet>
      ...
      >
        <textarea
          ...
          className="w-full bg-hover text-body text-base px-3 py-2 rounded border border-card h-24 resize-none"
        ...
        <div className="flex gap-2 mt-4">
          <button
            ...
            className="flex-1 py-2 bg-hover text-body rounded hover:bg-hover"
          >
            Cancel
          </button>
          <button
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 mt-6">
            <button
              ...
              className="flex-1 py-2 bg-hover text-body text-sm rounded hover:bg-hover"
            >
              Duplicate
            </button>
            <button
              onClick={() => { onDelete(selectedShape.id); setShowMoreSheet(false); }}
              className="flex-1 py-2 bg-red-600 text-body text-sm rounded hover:bg-red-700"
            >
              Delete
            </button>
          </div>
